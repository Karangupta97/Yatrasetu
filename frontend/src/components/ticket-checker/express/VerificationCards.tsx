...
function VerificationCards({
  ...
  return (
    <section className="tc-panel tc-verify-panel" aria-labelledby="tc-verify-title">
      <h2 id="tc-verify-title" className="tc-panel__title">Verify Ticket</h2>
      <div className={`tc-verify-split${verificationEnabled ? "" : " tc-verify-split--disabled"}`}>
        ...
        <div className="tc-verify-split__side tc-verify-split__side--qr">
          ...
          <p className="tc-verify-split__desc">Scan ticket QR code to verify ticket</p>
          <button
            type="button"
            ...
            disabled={disabled}
            aria-busy={qrLoading}
            aria-disabled={!verificationEnabled}
          >
            {qrLoading ? (
              <>
      ...
      {scanMessage && (
        <p className="tc-verify-toast" role="status" aria-live="polite">
          {scanMessage}
        </p>
      )}
    </section>
...
export default memo(VerificationCards);
